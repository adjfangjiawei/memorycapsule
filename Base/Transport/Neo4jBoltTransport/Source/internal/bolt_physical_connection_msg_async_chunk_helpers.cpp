...
#include "neo4j_bolt_transport/internal/async_utils_decl.h"           // For async_write_with_timeout, async_read_with_timeout
#include "neo4j_bolt_transport/internal/bolt_physical_connection.h"   // For BoltConnectionConfig, MAX_CHUNK_PAYLOAD_SIZE, etc.
#include "neo4j_bolt_transport/internal/i_async_context_callbacks.h"  // For IAsyncContextCallbacks

namespace neo4j_bolt_transport {
    namespace internal {

        // This namespace is just to group these static helpers, mimicking how they might be if part of BoltPhysicalConnection
        namespace detail_async_messaging_helpers {

            // Static helper for async _send_chunked_payload (internal to these static methods)
            static boost::asio::awaitable<boltprotocol::BoltError> _send_chunked_payload_async_static_helper(internal::ActiveAsyncStreamContext& stream_ctx,
                                                                                                             std::vector<uint8_t> payload,  // Pass by value for potential move
                ...
                auto write_to_stream_lambda = [&](const std::vector<uint8_t>& data_to_write, const std::string& op_detail) -> boost::asio::awaitable<boltprotocol::BoltError> {
                    ...
                    // Define DummyCallbacks locally within the lambda or pass it.
                    // Since it's used by async_write_with_timeout, it needs to be an
                    // IAsyncContextCallbacks*.
                    struct DummyCallbacksForWrite : public IAsyncContextCallbacks {
                        std::shared_ptr<spdlog::logger> lg;
                        uint64_t id_log = 0;  // Could get from stream_ctx if it had an ID
                    ...
                    write_result = co_await std::visit(
                        [&](auto& s_variant_member) -> boost::asio::awaitable<std::pair<boltprotocol::BoltError, std::size_t>> {
                            co_return co_await async_utils::async_write_with_timeout(&cb_write,  // Pass our dummy callback
                                                                                     s_variant_member,
                                                                                     boost::asio::buffer(data_to_write),
                                                                                     std::chrono::milliseconds(conn_config_ref.socket_write_timeout_ms),
                                                                                     op_name_prefix + op_detail);
                        },
                        stream_ctx.stream);

                    if (write_result.first != boltprotocol::BoltError::SUCCESS) co_return cb_write.get_last_error_code_from_async();  // Return error from callback
            ...
            // Static helper for async _receive_chunked_payload
            static boost::asio::awaitable<std::pair<boltprotocol::BoltError, std::vector<uint8_t>>> _receive_chunked_payload_async_static_helper(internal::ActiveAsyncStreamContext& stream_ctx,
                ...
                while (true) {
                    ...
                    if (chunk_payload_size == 0) break;  // EOM
