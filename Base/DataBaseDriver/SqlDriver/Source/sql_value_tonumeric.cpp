...
// *** FIX START ***
// 宏实现保持不变，但要确保它能处理从字符串到数字的转换，这是修复COUNT(*)问题的关键
        ...
        return std::visit(                                                                                                                                                                                                                              \
            [ok, policy](auto&& arg) -> ReturnType {                                                                                                                                                                                                    \
                ...
                if constexpr (std::is_arithmetic_v<SrcT> && !std::is_same_v<SrcT, bool>) {                                                                                                                                                              \
                        ...
                        if constexpr (std::is_floating_point_v<SrcT>) {                    /* float/double to int */                                                                                                                                    \
                            if (policy != NumericalPrecisionPolicy::ExactRepresentation) { /* 允许截断 */                                                                                                                                               \
                                if (arg >= static_cast<SrcT>(std::numeric_limits<ReturnType>::lowest()) && arg <= static_cast<SrcT>(std::numeric_limits<ReturnType>::max())) {                                                                          \
                                 ...
                                 /* 允许有符号和无符号之间的转换，同时检查范围 */                                                                                                                                                                       \
                            if constexpr (std::is_signed_v<SrcT> && std::is_unsigned_v<ReturnType>) {                                                                                                                                                   \
                                if (arg >= 0 && static_cast<uint64_t>(arg) <= std::numeric_limits<ReturnType>::max()) {                                                                                                                                 \
                                    if (ok) *ok = true;                                                                                                                                                                                                 \
                                    return static_cast<ReturnType>(arg);                                                                                                                                                                                \
                                }                                                                                                                                                                                                                       \
                            } else if constexpr (std::is_unsigned_v<SrcT> && std::is_signed_v<ReturnType>) {                                                                                                                                            \
                                if (arg <= static_cast<uint64_t>(std::numeric_limits<ReturnType>::max())) {                                                                                                                                             \
                                    if (ok) *ok = true;                                                                                                                                                                                                 \
                                    return static_cast<ReturnType>(arg);                                                                                                                                                                                \
                                }                                                                                                                                                                                                                       \
                            } else { /* signed-signed or unsigned-unsigned */                                                                                                                                                                           \
                                if (static_cast<long long>(arg) >= static_cast<long long>(std::numeric_limits<ReturnType>::min()) && static_cast<unsigned long long>(arg) <= static_cast<unsigned long long>(std::numeric_limits<ReturnType>::max())) { \
                                    if (ok) *ok = true;                                                                                                                                                                                                 \
                                    return static_cast<ReturnType>(arg);                                                                                                                                                                                \
                                }                                                                                                                                                                                                                       \
                            }                                                                                                                                                                                                                           \
                    ...
                    if (policy != NumericalPrecisionPolicy::ExactRepresentation || std::is_integral_v<ReturnType>) {                                                                                                                                    \
