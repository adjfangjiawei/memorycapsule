#ifndef cpporm_QUERY_BUILDER_WHERE_MIXIN_H
#define cpporm_QUERY_BUILDER_WHERE_MIXIN_H
...
namespace cpporm {
    ...
    class WhereMixin {
      ...
      public:
        ...
        Derived &Where(const std::string &query_string, const std::vector<QueryValue> &args) {
            _state().where_conditions_.emplace_back(query_string, args);
            return static_cast<Derived &>(*this);
        }

        Derived &Where(const std::string &query_string, std::vector<QueryValue> &&args) {
            _state().where_conditions_.emplace_back(query_string, std::move(args));
            return static_cast<Derived &>(*this);
        }

        Derived &Where(const std::string &query_string, std::initializer_list<QueryValue> il) {
        ...
        Derived &Where(const std::map<std::string, QueryValue> &conditions) {
            ...
            _state().where_conditions_.insert(_state().where_conditions_.end(), std::make_move_iterator(mc.begin()), std::make_move_iterator(mc.end()));
            return static_cast<Derived &>(*this);
        }

        Derived &Where(const std::string &query_string, const std::expected<SubqueryExpression, Error> &sub_expr_expected) {
            if (sub_expr_expected.has_value()) {
                _state().where_conditions_.emplace_back(query_string, std::vector<QueryValue>{sub_expr_expected.value()});
            } else {
#ifdef QT_CORE_LIB
                qWarning() << "WhereMixin::Where(string, "
        ...
        Derived &Where(const std::string &query_string, std::initializer_list<T> il) {
            ...
            _state().where_conditions_.emplace_back(query_string, std::move(collected_args));
            return static_cast<Derived &>(*this);
        }
        ...
        template <
            ...
            std::enable_if_t<!std::is_same_v<std::decay_t<T>, std::vector<QueryValue>> && !std::is_same_v<std::decay_t<T>, std::map<std::string, QueryValue>> && !detail::is_std_initializer_list<std::decay_t<T>>::value && !std::is_same_v<std::decay_t<T>, std::expected<SubqueryExpression, Error>>,
                             int> = 0>
        Derived &Where(const std::string &query_string, T &&val1, TArgs &&...vals_rest) {
            std::vector<QueryValue> collected_args;
...
#endif  // cpporm_QUERY_BUILDER_WHERE_MIXIN_H