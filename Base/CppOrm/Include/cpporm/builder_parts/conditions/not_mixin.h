#ifndef cpporm_QUERY_BUILDER_NOT_MIXIN_H
#define cpporm_QUERY_BUILDER_NOT_MIXIN_H
...
namespace cpporm {
    ...
    class NotMixin {
      ...
      public:
        ...
        Derived &Not(const std::string &query_string, const std::vector<QueryValue> &args) {
            _state().not_conditions_.emplace_back(query_string, args);
            return static_cast<Derived &>(*this);
        }

        Derived &Not(const std::string &query_string, std::vector<QueryValue> &&args) {
            _state().not_conditions_.emplace_back(query_string, std::move(args));
            return static_cast<Derived &>(*this);
        }

        Derived &Not(const std::string &query_string, std::initializer_list<QueryValue> il) {
        ...
        Derived &Not(const std::map<std::string, QueryValue> &conditions) {
            ...
            _state().not_conditions_.insert(_state().not_conditions_.end(), std::make_move_iterator(mc.begin()), std::make_move_iterator(mc.end()));
            return static_cast<Derived &>(*this);
        }

        Derived &Not(const std::string &query_string, const std::expected<SubqueryExpression, Error> &sub_expr_expected) {
            if (sub_expr_expected.has_value()) {
                _state().not_conditions_.emplace_back(query_string, std::vector<QueryValue>{sub_expr_expected.value()});
            } else {
#ifdef QT_CORE_LIB
                qWarning() << "NotMixin::Not(string, "
        ...
        Derived &Not(const std::string &query_string, std::initializer_list<T> il) {
            ...
            _state().not_conditions_.emplace_back(query_string, std::move(collected_args));
            return static_cast<Derived &>(*this);
        }
        ...
        template <
            ...
            std::enable_if_t<!std::is_same_v<std::decay_t<T>, std::vector<QueryValue>> && !std::is_same_v<std::decay_t<T>, std::map<std::string, QueryValue>> && !detail::is_std_initializer_list<std::decay_t<T>>::value && !std::is_same_v<std::decay_t<T>, std::expected<SubqueryExpression, Error>>,
                             int> = 0>
        Derived &Not(const std::string &query_string, T &&val1, TArgs &&...vals_rest) {
            std::vector<QueryValue> collected_args;
...
#endif  // cpporm_QUERY_BUILDER_NOT_MIXIN_H