...
boltprotocol::BoltError simulate_client_hello(const boltprotocol::versions::Version& client_simulated_target_version, std::vector<uint8_t>& out_raw_bytes) {
    ...
    try {
        ...
        if (client_simulated_target_version < V5_1) {
        ...
        if (!(client_simulated_target_version < V5_3)) {  // Equivalent to client_simulated_target_version >= V5_3
...
int main() {
    ...
    versions::Version server_assumed_negotiated_version = versions::V5_3;
    ...
    RunMessageParams actual_run_params;
    {
        ...
        err = deserialize_run_message_request(run_reader, actual_run_params);
        if (err != BoltError::SUCCESS) {
